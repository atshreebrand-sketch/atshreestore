...
supabaseClient.auth.signUp = async (...args) => { const remaining=authCooldownRemaining(AUTH_SIGNUP_LOCK); if(remaining>0){const seconds=Math.ceil(remaining/1000);return {data:{user:null,session:null},error:new Error(`Please wait ${seconds} seconds before trying account creation again. If you already created the account, check your email or sign in instead.`)}} markAuthEmailAttempt(AUTH_SIGNUP_LOCK); const result=await originalSignUp(...args); if(result?.error&&/rate limit|429|too many/i.test(result.error.message||"")){return {...result,error:new Error("Email sending is temporarily rate-limited. Please wait before trying again. ATSHREE should use custom SMTP for production customer confirmation emails.")}} return result; };
...
// Admin indicator: database is_admin is authoritative. This only adds navigation for authorized admins.
async function addAdminAccessButton(){
  try{
    ...
    if(document.querySelector('[data-atshree-admin-link]'))return;
    const navRight=document.querySelector('.navRight');
    if(navRight){const link=document.createElement('a');link.href='/admin.html';link.className='accountLink';link.dataset.atshreeAdminLink='true';link.textContent='ADMIN';navRight.insertBefore(link,navRight.firstChild);}
    const profileCard=document.querySelector('#accountArea .accountCard');
    if(profileCard){const link=document.createElement('a');link.href='/admin.html';link.className='btn';link.dataset.atshreeAdminLink='true';link.textContent='OPEN ADMIN DASHBOARD';link.style.marginTop='14px';profileCard.appendChild(link);}
  }catch(e){console.warn('Admin access indicator unavailable',e);}
}
