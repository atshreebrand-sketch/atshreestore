...
function renderAccount() {
  ...
  if (!customer) { area.innerHTML = `<div class="accountCard"><div class="ey">ATSHREE ACCOUNT</div><h2>Welcome back.</h2><p>Create a customer profile to keep your details, addresses, orders and rewards together.</p><form class="form" onsubmit="event.preventDefault();createCustomer(this)"><input name="name" placeholder="Full name" required><input name="email" type="email" placeholder="Email address" required><input name="mobile" placeholder="Mobile number" required><button class="btn">CREATE ACCOUNT</button></form><p class="accountNote">Secure authentication will be connected with the production database before launch.</p></div>`; return; }
  const rewards = getRewards(), addresses = getAddresses(), orders = getOrders();
  area.innerHTML = `<div class="accountGrid">
    <div class="accountCard"><div class="ey">MY PROFILE</div><h2>${customer.name}</h2><p>${customer.email}<br>${customer.mobile || ""}</p><button class="textBtn" onclick="editProfile()">EDIT PROFILE</button><button class="textBtn" onclick="logoutCustomer()">LOG OUT</button></div>
    <div class="accountCard"><div class="ey">ATSHREE REWARDS</div><h2>${rewardTier(rewards.points)}</h2><div class="rewardPoints">${rewards.points}<span>POINTS</span></div><p>Earn 1 point for every ₹100 spent.</p><a class="btn" href="/rewards.html">VIEW REWARDS</a></div>
  </div>
  <div class="accountGrid accountLower">
    <div class="accountCard"><div class="ey">MY ADDRESSES</div><h2>Delivery details</h2><p>${addresses.length ? `${addresses.length} saved address${addresses.length > 1 ? "es" : ""}.` : "No saved address yet."}</p><div class="addressList">${addresses.map((a,i) => `<div class="addressItem"><strong>${a.label}</strong><br>${a.name}<br>${a.line1}${a.line2 ? ", " + a.line2 : ""}<br>${a.city}, ${a.state} — ${a.pin}<br>Mobile: ${a.mobile}<button class="textBtn" onclick="removeAddress(${i})">REMOVE</button></div>`).join("")}</div><h3>Add a new address</h3><form class="form" onsubmit="event.preventDefault();addAddress(this)"><input name="label" placeholder="Label (Home / Office)" required><input name="name" value="${customer.name}" placeholder="Full name" required><input name="mobile" value="${customer.mobile || ""}" placeholder="Mobile number" required><input name="line1" placeholder="House / Flat / Street" required><input name="line2" placeholder="Area / Landmark"><div class="checkoutTwo"><input name="city" placeholder="City" required><input name="state" placeholder="State" required></div><input name="pin" placeholder="PIN code" inputmode="numeric" required><button class="btn">SAVE ADDRESS</button></form></div>
    <div class="accountCard"><div class="ey">MY ORDERS</div><h2>Order history</h2>${orders.length ? orders.map(o => `<div class="orderMini"><strong>${o.number}</strong><span>${o.status}</span><small>${o.items} item${o.items === 1 ? "" : "s"} · ${money(o.total)}</small></div>`).join("") : `<p>No orders yet.</p><a class="btn" href="/shop.html">START SHOPPING</a>`}</div>
  </div>`;
}
